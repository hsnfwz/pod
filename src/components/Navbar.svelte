...
<nav class="flex flex-col bg-slate-900 rounded-lg sticky top-4 h-min p-4 gap-4 w-60 sm:w-80">
  ...
  <a class={`flex gap-2 items-center px-2 py-1 hover:underline hover:text-sky-500 ${$page.url.pathname.includes(`/u/${$profile.username}`) ? 'pointer-events-none' : ''}`} href={`/u/${$profile.username}`}>
  ...
  <div class="flex gap-2">
    <span class="flex items-center gap-2 px-2 py-1">
      ...
      <span class="font-bold">{$profile.coin_count}</span>
    </span>
    <span class="flex items-center gap-2 px-2 py-1">
      ...
      <span class="font-bold text-amber-500">{$profile.super_coin_count}</span>
    </span>
  </div>
