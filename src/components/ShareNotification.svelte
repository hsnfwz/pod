...
<div class="flex flex-col bg-slate-900 p-4 rounded-lg gap-4 w-60 sm:w-80">
  <div class="flex gap-2 items-center">
    <a class="hover:underline flex items-center gap-2" href={`/u/${profileShare.sender_profile_id.username}`}>
      <div class="w-8 h-8 rounded-full bg-slate-500"></div>
      <TextBase>{profileShare.sender_profile_id.username}</TextBase>
    ...
    <span>sent a <a class="text-sky-500 hover:underline" href={`/p/${profileShare.post_id}`}>post</a></span>
  </div>
